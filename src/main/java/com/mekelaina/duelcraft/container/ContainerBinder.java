...
public class ContainerBinder extends CommonContainer
{
    ...
    private static final int XSTART_DECKBOX = -12;
    private static final int YSTART_DECKBOX = -12;
        ...
        for (int y = 0; y < numRows; ++y)
        {
            for (int x = 0; x < numColumns; ++x)
            {
                ...
                this.addSlotToContainer(new SlotItemHandler(binderInv, index, xPos, yPos)
                {
                    ...
                    {
                        return stack.getItem() instanceof ItemCard && stack.hasTagCompound();
                    }
                });
            }
        }

        this.addSlotToContainer(new SlotItemHandler(binderInv, 60, 80, 12)
        {
            @Override
            public boolean isItemValid(ItemStack stack)
            {
                return stack.getItem() instanceof ItemDeckbox;
            }
